"""
OD流量预测模型 - 针对纽约出租车数据优化版本

主要修改：
1. 数据归一化策略优化：
   - 使用全局统计量进行标准化，而非按样本归一化
   - 对OD流量使用log1p变换处理小值数据

2. 模型参数调整：
   - batch_size: 32 -> 16 (提高训练稳定性)
   - lr: 5e-5 -> 1e-4 (适应小规模数据)
   - hidden_dim: 128 -> 64 (减少过拟合风险)
   - epochs: 100 -> 200 (增加训练轮数)
   - weight_decay: 1e-6 -> 1e-5 (增强正则化)
